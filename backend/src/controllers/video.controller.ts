...
export const uploadVideoController = async (req: Request, res: Response) => {
    ...
    const requestedTitle = (req.body as { title?: string }).title?.trim();
    const title = requestedTitle || req.file.originalname || "Untitled Video";
    const rawDescription = (req.body as { description?: string }).description;
    ...
    let videoRecord;
    try {
        const createData: { title: string; description?: string; originalFilename: string; hlsPath: string; playlistUrl: string } = {
            title,
            originalFilename,
            hlsPath: outputPath,
            playlistUrl: "",
    ...
    processVideoForHls(videoPath, outputPath, async (error, masterPlaylist) => {
        if (error || !masterPlaylist) {
            console.log(
                ...
                error?.message || "No playlist generated"
            );
